...
colors = ['navy', 'turquoise', 'darkorange', 'blue', 'green','red', 'cyan', 'magenta' , 'yellow' , 'black']
...
mnist_matrix = manifold.SpectralEmbedding(n_components=9).fit_transform(data)
print(mnist_matrix)
...
print("Apres normalisation \b", mnist_matrix) #Afficher le cluster
...
for index, (name, estimator) in enumerate(estimators.items()):
		...
		for n, color in enumerate(colors):
				data = mnist.data[mnist.target == n]
				plt.scatter(data[:, 0], data[:, 1], s=0.8, color=color,
										label=mnist.target_names[n])
